...
(function ($PS) {
  ...
  var options = function options(v) {
...
(function ($PS) {
  ...
  var getUserMedia = function getUserMedia($1) {
    return Control_Promise.toAffE($foreign["_getUserMedia"]($1));
  };

  var getTracks = function getTracks(dictMonadEffect) {
    return function (ms) {
      return Effect_Class.liftEffect(dictMonadEffect)($foreign["_getTracks"](ms));
    };
  };
...
(function ($PS) {
  ...
  var optWith = function optWith(f) {
    return function ($0) {
...
})(PS["WebRTC.RTCPeerConnection"] = PS["WebRTC.RTCPeerConnection"] || {});
...
(function ($PS) {
  ...
  $PS["WebRTC.RTCSessionDescription"] = $PS["WebRTC.RTCSessionDescription"] || {};
  var exports = $PS["WebRTC.RTCSessionDescription"];
  var Control_Applicative = $PS["Control.Applicative"];
  var Control_Bind = $PS["Control.Bind"];
  var Control_Monad_Except_Trans = $PS["Control.Monad.Except.Trans"];
  var Data_Boolean = $PS["Data.Boolean"];
  ...
  var Pranswer = function () {
    function Pranswer() {}

    ;
    Pranswer.value = new Pranswer();
    return Pranswer;
  }();
  ...
  var Rollback = function () {
    ...
    ;
    Rollback.value = new Rollback();
    return Rollback;
  }();

  var strToRTCSdpType = function strToRTCSdpType(str) {
    if (str === "offer") {
      return Offer.value;
    }
    ...
    ;
    throw new Error("Failed pattern match at WebRTC.RTCSessionDescription (line 28, column 1 - line 28, column 40): " + [str.constructor.name]);
  };

  var showRTCSdpType = new Data_Show.Show(function (v) {
    ...
    if (v instanceof Pranswer) {
      return "pranswer";
    }

    ;

    if (v instanceof Rollback) {
      return "rollback";
    }

    ;
    throw new Error("Failed pattern match at WebRTC.RTCSessionDescription (line 14, column 1 - line 18, column 29): " + [v.constructor.name]);
  });

  var readRTCSessionDescription = function readRTCSessionDescription(value) {
    return Control_Bind.bind(Control_Monad_Except_Trans.bindExceptT(Data_Identity.monadIdentity))(Control_Bind.bind(Control_Monad_Except_Trans.bindExceptT(Data_Identity.monadIdentity))(Foreign_Index.ix(Foreign_Index.indexableForeign)(Foreign_Index.indexString)(value)("type"))(Foreign.readString))(function (type_) {
      return Control_Bind.bind(Control_Monad_Except_Trans.bindExceptT(Data_Identity.monadIdentity))(Control_Bind.bind(Control_Monad_Except_Trans.bindExceptT(Data_Identity.monadIdentity))(Foreign_Index.ix(Foreign_Index.indexableForeign)(Foreign_Index.indexString)(value)("sdp"))(Foreign.readString))(function (sdp) {
  ...
  exports["readRTCSessionDescription"] = readRTCSessionDescription;
  exports["showRTCSdpType"] = showRTCSdpType;
})(PS);
...
(function ($PS) {
  ...
  $PS["WebRTC.RTCPeerConnection"] = $PS["WebRTC.RTCPeerConnection"] || {};
  var exports = $PS["WebRTC.RTCPeerConnection"];
  var $foreign = $PS["WebRTC.RTCPeerConnection"];
  var Control_Applicative = $PS["Control.Applicative"];
  var Control_Bind = $PS["Control.Bind"];
  var Control_Monad_Except = $PS["Control.Monad.Except"];
  var Control_Promise = $PS["Control.Promise"];
  ...
  var Effect_Class = $PS["Effect.Class"];
  var Foreign = $PS["Foreign"];
  var WebRTC_RTCSessionDescription = $PS["WebRTC.RTCSessionDescription"];

  var RTCOfferOptions = function RTCOfferOptions(x) {
    return x;
  };

  var setRemoteDescription = function setRemoteDescription(conn) {
    return function (v) {
      var descForeign = Foreign.unsafeToForeign({
        type: Data_Show.show(WebRTC_RTCSessionDescription.showRTCSdpType)(v.type),
        sdp: v.sdp
      });
      return Control_Promise.toAffE($foreign["_setRemoteDescription"](conn)(descForeign));
    };
  };

  var setLocalDescription = function setLocalDescription(conn) {
    return function (v) {
      var descForeign = Foreign.unsafeToForeign({
        type: Data_Show.show(WebRTC_RTCSessionDescription.showRTCSdpType)(v.type),
        sdp: v.sdp
      });
      return Control_Promise.toAffE($foreign["_setLocalDescription"](conn)(descForeign));
    };
  };

  var newtypeRTCOfferOptions = new Data_Newtype.Newtype(function (n) {
    return n;
  }, RTCOfferOptions);

  var newRTCPeerConnection = function newRTCPeerConnection(dictMonadEffect) {
    return function (config) {
      return Effect_Class.liftEffect(dictMonadEffect)($foreign["_newRTCPeerConnection"](Data_Options.options(config)));
    };
  };

  var createOffer = function createOffer(conn) {
    return function (opts) {
      var fOpts = Foreign.unsafeToForeign(Data_Newtype.unwrap(newtypeRTCOfferOptions)(opts));
      return Control_Bind.bind(Effect_Aff.bindAff)(Control_Promise.toAffE($foreign["_createOffer"](conn)(fOpts)))(function (resolv) {
        return Control_Applicative.pure(Effect_Aff.applicativeAff)(Control_Monad_Except.runExcept(WebRTC_RTCSessionDescription.readRTCSessionDescription(resolv)));
      });
    };
  };

  var createAnswer = function createAnswer(conn) {
    return Control_Bind.bind(Effect_Aff.bindAff)(Control_Promise.toAffE($foreign["_createAnswer"](conn)))(function (resolv) {
      return Control_Applicative.pure(Effect_Aff.applicativeAff)(Control_Monad_Except.runExcept(WebRTC_RTCSessionDescription.readRTCSessionDescription(resolv)));
    });
  };
  ...
  var addTrack = function addTrack(dictMonadEffect) {
    return function (conn) {
      return function (track) {
        return function (stream) {
          return Effect_Class.liftEffect(dictMonadEffect)($foreign["_addTrack"](conn)(track)(stream));
        };
      };
    };
  };
...
(function ($PS) {
  ...
  var WebRTC_RTCPeerConnection = $PS["WebRTC.RTCPeerConnection"];
  ...
  var main = Effect_Aff.launchAff_(Control_Bind.bind(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.newRTCPeerConnection(Effect_Aff.monadEffectAff)(rtcConfig))(function (pc1) {
    return Control_Bind.bind(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.newRTCPeerConnection(Effect_Aff.monadEffectAff)(rtcConfig))(function (pc2) {
      var offerOptions = {
        iceRestart: false,
      ...
      return Control_Bind.bind(Effect_Aff.bindAff)(WebRTC_MediaStream.getUserMedia({
        ...
        return Control_Bind.bind(Effect_Aff.bindAff)(WebRTC_MediaStream.getTracks(Effect_Aff.monadEffectAff)(stream))(function (tracks) {
          return Control_Bind.discard(Control_Bind.discardUnit)(Effect_Aff.bindAff)(Data_Foldable.for_(Effect_Aff.applicativeAff)(Data_Foldable.foldableArray)(tracks)(function (t) {
            return WebRTC_RTCPeerConnection.addTrack(Effect_Aff.monadEffectAff)(pc1)(t)(stream);
          }))(function () {
            return Control_Bind.bind(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.createOffer(pc1)(offerOptions))(function (offer) {
              if (offer instanceof Data_Either.Right) {
                return Control_Bind.discard(Control_Bind.discardUnit)(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.setLocalDescription(pc1)(offer.value0))(function () {
                  return Control_Bind.discard(Control_Bind.discardUnit)(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.setRemoteDescription(pc2)(offer.value0))(function () {
                    return Control_Bind.bind(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.createAnswer(pc2))(function (answer) {
                      if (answer instanceof Data_Either.Right) {
                        return Control_Bind.discard(Control_Bind.discardUnit)(Effect_Aff.bindAff)(WebRTC_RTCPeerConnection.setLocalDescription(pc2)(answer.value0))(function () {
                          return WebRTC_RTCPeerConnection.setRemoteDescription(pc1)(answer.value0);
                        });
                      }

                      ;

                      if (answer instanceof Data_Either.Left) {
                        return Effect_Class_Console.logShow(Effect_Aff.monadEffectAff)(Data_List_Types.showNonEmptyList(Foreign.showForeignError))(answer.value0);
                      }

                      ;
                      throw new Error("Failed pattern match at Example.Main (line 77, column 7 - line 84, column 32): " + [answer.constructor.name]);
                    });
                  });
                });
              }
              ...
              if (offer instanceof Data_Either.Left) {
                return Effect_Class_Console.logShow(Effect_Aff.monadEffectAff)(Data_List_Types.showNonEmptyList(Foreign.showForeignError))(offer.value0);
              }

              ;
              throw new Error("Failed pattern match at Example.Main (line 66, column 3 - line 85, column 28): " + [offer.constructor.name]);
            });
          });
        });
      });
...
//# sourceMappingURL=/example/dist/index.js.map